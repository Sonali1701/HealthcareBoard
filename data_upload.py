...
def _post_resume(server: str, token: str, path: Path) -> dict:
    """POST one résumé to the job board's /api/admin/import (stdlib only)."""
    import urllib.error
    import urllib.request
    import uuid
    ...
    req = urllib.request.Request(
        server.rstrip("/") + "/api/admin/import", data=body, method="POST")
    req.add_header("Content-Type", f"multipart/form-data; boundary={boundary}")
    req.add_header("X-Import-Token", token)
    try:
...
def import_folder_remote(args: argparse.Namespace) -> dict:
    """Thin-client import: send each file to the server, which owns dedup + junk
    filtering + storage + insert. No R2/DB credentials needed on this machine."""
    server = args.server
    token = args.token or os.environ.get("IMPORT_API_TOKEN", "")
    if not token:
        raise SystemExit("ERROR: pass --token or set IMPORT_API_TOKEN in .env for --server mode.")

    files = _iter_files(args.folder, args.limit)
    manifest_path = _env_path(args.manifest)
    done_hashes = set() if args.ignore_manifest else _load_done_hashes(manifest_path)
    print(f"Server: {server}  (server-side dedup + junk filtering)")
    print(f"Found {len(files)} supported resume file(s).")

    stats = {"created": 0, "duplicate": 0, "skipped": 0, "failed": 0, "total": len(files)}
    started = monotonic()
    for i, path in enumerate(files, start=1):
        try:
            digest = _sha256_file(path)
            if digest in done_hashes:
                ...
                continue
            if args.dry_run:
                print(f"[{i}/{len(files)}] WOULD POST {path.name}")
                continue

            result = _post_resume(server, token, path)
            status = result.get("status")
            if status == "created":
                stats["created"] += 1
                tag = "IMPORTED" if result.get("listable") else "HIDDEN-junk"
                print(f"[{i}/{len(files)}] {tag:11s} {result.get('name', '?'):24.24s} <- {path.name}")
                _append_manifest(manifest_path, {
                    "status": "imported", "sha256": digest, "file": str(path),
                    "profile_id": result.get("profile_id")})
            elif status == "duplicate":
                stats["duplicate"] += 1
                print(f"[{i}/{len(files)}] SKIP dup-{result.get('reason')} {path.name}")
                _append_manifest(manifest_path, {
                    "status": "imported", "sha256": digest, "file": str(path),
                    "profile_id": result.get("profile_id"), "duplicate": True})
            else:
                stats["failed"] += 1
                print(f"[{i}/{len(files)}] FAIL {path.name}: {result}", file=sys.stderr)
...
def main() -> int:
    ...
    parser.add_argument("--server", default=None,
                        help="Import via the job board API (thin client) instead of writing "
                             "to R2/DB directly, e.g. https://board.example.com. The server "
                             "does dedup + junk filtering, so no R2/DB creds are needed here.")
    parser.add_argument("--token", default=None,
                        help="X-Import-Token for --server mode (else IMPORT_API_TOKEN from .env)")
    ...
    if args.server:
        _load_env(getattr(args, "env_file", ".env"))   # only to read IMPORT_API_TOKEN
        import_folder_remote(args)
        return 0
